...
class TestTerminalDropDurability:
    ...
    def test_terminal_drop_persists_payload_durably(
        ...
        # The pre-flight settles the durable row as dropped; the delivery
        # ladder's ``finally`` may then issue a release, which is a no-op on
        # a row that is no longer pending. Pin the disposition, not the ladder.
        assert calls["drop"] == 1
