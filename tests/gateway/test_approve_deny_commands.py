...
class TestBlockingGatewayApproval:
    ...
    def test_unregister_cancels_unanswered_prompt(self):
        """Turn teardown records a withdrawal, not a user denial. A result that
        raced in ahead of teardown is kept."""
        from tools.approval import register_gateway_notify, unregister_gateway_notify, _gateway_queues
        from tools.approval_gateway_wait import _ApprovalEntry
        session_key = "test-teardown-cancelled"
        register_gateway_notify(session_key, lambda d: None)
        ...
        assert pending.cancelled == "the turn ended before the prompt was answered"
        assert raced.event.is_set()
        assert raced.result == "once"
        assert raced.cancelled is None
