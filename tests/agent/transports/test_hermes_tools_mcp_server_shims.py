...
class TestMemoryShim:
    ...
    def test_shim_unregistered_when_external_provider_configured(
        ...
        names = [name for name, _desc, _schema, _fn in stateless_shim_definitions()]
...
class TestSessionSearchShim:
    ...
    def test_session_id_read_from_canonical_env(self, tmp_hermes_home, monkeypatch):
        ...
        # the `_SESSION_ID_ENV` note in the shims module). A bespoke name has a
    ...
    def test_native_zero_hit_relaxation_reaches_shim(self, tmp_hermes_home, monkeypatch):
        # The native SessionDB search owns zero-hit OR relaxation now; the shim
        # must inherit it without a second, divergent retry layer.
        db_path = tmp_hermes_home / "state.db"
        self._seed_db(db_path)
        monkeypatch.setenv("HERMES_MCP_STATE_DB", str(db_path))
        out = json.loads(
    ...
    def test_discovery_controls_forward_to_native_tool(self, tmp_hermes_home, monkeypatch):
        db_path = tmp_hermes_home / "state.db"
        self._seed_db(db_path)
        monkeypatch.setenv("HERMES_MCP_STATE_DB", str(db_path))
        ...
        monkeypatch.setattr(
            sst,
            "session_search",
            lambda **kwargs: (captured.update(kwargs), json.dumps({"success": True}))[1],
        )
        dispatch_session_search(
            {
                "query": "auth",
                "after": "7d",
                "before": "2026-09-15",
                "exclude_session_ids": ["sess-seen-1"],
            }
        )

        assert captured["after"] == "7d"
        assert captured["before"] == "2026-09-15"
        assert captured["exclude_session_ids"] == ["sess-seen-1"]
...
class TestShimRegistration:
    ...
    def test_shim_signatures_carry_the_registry_schema(self, tmp_hermes_home):
        ...
        for name, _desc, schema, _fn in stateless_shim_definitions():
        ...
        shim_schemas = {n: s for n, _d, s, _f in stateless_shim_definitions()}
