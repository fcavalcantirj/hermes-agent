...
class TestDetectProviderForModel:
    ...
    def test_claude_agent_sdk_pin_not_hijacked_by_openrouter_fallback(self):
        ...
        with patch(
            "hermes_cli.models.fetch_openrouter_models",
            return_value=[("anthropic/claude-sonnet-5", "Claude Sonnet")],
        ):
    ...
    def test_non_anthropic_model_still_switches_off_claude_agent_sdk(self):
        """An authenticated non-Anthropic target still switches normally;
        the subscription pin protects Claude ids, not the whole catalog."""
        with patch(
            "hermes_cli.models_detect.provider_has_credentials", return_value=True
        ):
