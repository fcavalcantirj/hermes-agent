...
def test_human_single_query_failed_turn_exits_nonzero(monkeypatch):
    ...
    class FakeCLI:
        ...
        def chat(self, _query, images=None):
            # The one-shot path consumes the raw result, including early failures.
            self._last_turn_result = {"failed": True, "failure_reason": "startup"}
            return None
...
def _kanban_fake_cli(failure_reason):
    class FakeCLI:
        ...
        def chat(self, _query, images=None):
            self._last_turn_result = {"failed": True, "failure_reason": failure_reason}
            return None
