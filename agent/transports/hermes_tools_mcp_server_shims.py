...
server without touching that refusal:
  ...
  session_search → ``SessionDB(read_only=True)`` over the state DB (never a
                   writable handle in a model-facing subprocess). Search
                   semantics, including zero-hit OR relaxation and discovery
                   bounds, stay owned by the native tool.
...
def dispatch_session_search(kwargs: dict[str, Any]) -> str:
    ...
    def _run(query: str) -> str:
        return session_search_tool.session_search(
            ...
            exclude_session_ids=kwargs.get("exclude_session_ids"),
            db=db,
            current_session_id=os.environ.get(_SESSION_ID_ENV, "").strip() or None,
        )

    try:
        return _run(kwargs.get("query") or "")
    finally:
        try:
            db.close()
        except Exception:
