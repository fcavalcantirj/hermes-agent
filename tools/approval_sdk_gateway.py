...
def _sdk_decision_outcome(decision: dict, session_key: str):
    ...
    if decision.get("cancelled"):
        # A withdrawn prompt is never an operator denial. Keep the turn-end reason
        # stable for SDK callers, while other cancellation causes remain unattributed.
        reason = ("approval expired (turn ended)"
                  if decision["cancelled"] == "the turn ended before the prompt was answered"
                  else "approval withdrawn before operator response")
        return {"choice": "deny", "reason": reason}
    ...
    if choice in (None, "expired"):
        # Defensive compatibility for older waiters that stamped an expired choice.
